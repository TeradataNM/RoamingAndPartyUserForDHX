...
class SparkConfig {
  def getConf = {
    ...
    降低了由于reduce task过多导致每个Excutor中存在的fetch线程太多而导致的fetch超时，另外降低内存的占用。*/
      ...
      .set("spark.hadoop.hadoop.security.bdoc.access.id", "d690843cba5facbce853")
      .set("spark.hadoop.hadoop.security.bdoc.access.key", "2e6212edd46b91aa3055baa965b01f6f8cf9f482")
      //      .set("spark.hadoop.hadoop.security.proxy.user", "sunjiafeng")
      .set("queue", "root.bdoc.renter_1.renter_10.dev_556")
  }
}
